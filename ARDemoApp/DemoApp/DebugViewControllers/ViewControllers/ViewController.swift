...
extension ViewController: MenuViewSelect {
    func selectMenuView(selectString: String) {
        let path = Bundle.main.path(forResource: "Resources.scnassets", ofType: nil)
        VirtualModelEntityLoader.loadAsync(name: selectString, in: Bundle(path: path!)) { [weak self] virtual in
